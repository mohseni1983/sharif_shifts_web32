...
class _reserveShiftState extends State<reserveShift> {
                      ...
                      children: <Widget>[
                        Positioned(
                          ...
                          child: FloatingActionButton(
                            ...
                            elevation: 3.0,
                            child: Column(
                              children: [Icon(Icons.save), Text('ذخیره')],
                            ),
                                ...
                                child: listState == 1
                                    ? ListView.builder(
                                        ...
                                        itemBuilder: (context, index) {
                                          ...
                                          int itempcount =
                                              item.shiftPersons != null
                                                  ? item.shiftPersons.length
                                                  : 0;
                                          int itemshiftcount =
                                              item.shiftQuantity - itempcount;
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Directionality(
              textDirection: TextDirection.rtl,
              child: new Column(
                children: [Text('برخی شیفت ها به علت پر شدن ظرفیت ثبت نشدند.')],
              ),
            ),
            actions: [
              new MaterialButton(
  ...
  void addShift(int shiftId, int madadkarId) async {
    var response = await http.post(
      '${globalVars.s_url}api/Job/AddShiftForMadadkar?shiftid=$shiftId&madadkarId=$madadkarId',
    );
  ...
  void removeShift(int shiftId, int madadkarId) async {
    var response = await http.post(
      '${globalVars.s_url}api/Job/RemoveShiftForMadadkar?shiftid=$shiftId&madadkarId=$madadkarId',
    );
